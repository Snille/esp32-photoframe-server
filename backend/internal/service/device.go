...
func (s *DeviceService) AddDevice(host string, enableCollage, showDate, showPhotoDate, showWeather bool, weatherLat, weatherLon float64, layout string, displayMode string, showCalendar bool, calendarID string, dateFormat string, showBattery bool, displayOrder string, immichAlbumIDs string, onThisDay, favoritesOnly bool, overlay model.OverlaySettings) (*model.Device, error) {
	...
	device := &model.Device{
		...
		LowBatteryWarnText:          overlay.LowBatteryWarnText,
		LowBatteryWarnPosition:      model.NormalizeOverlayPosition(overlay.LowBatteryWarnPosition, "top-center"),
		CriticalBatteryWarnPercent:  model.NormalizeWarnPercent(overlay.CriticalBatteryWarnPercent, 10),
		CriticalBatteryWarnText:     overlay.CriticalBatteryWarnText,
		CriticalBatteryWarnPosition: model.NormalizeVerticalBand(overlay.CriticalBatteryWarnPosition),
...
func (s *DeviceService) UpdateDevice(id uint, name, host, orientation string, enableCollage, showDate, showPhotoDate, showWeather bool, weatherLat, weatherLon float64, aiProvider, aiModel, aiPrompt string, layout string, displayMode string, showCalendar bool, calendarID string, dateFormat string, showBattery bool, displayOrder string, immichAlbumIDs string, onThisDay, favoritesOnly bool, overlay model.OverlaySettings) (*model.Device, error) {
	...
	device.LowBatteryWarnText = overlay.LowBatteryWarnText
	device.LowBatteryWarnPosition = model.NormalizeOverlayPosition(overlay.LowBatteryWarnPosition, "top-center")
	device.CriticalBatteryWarnPercent = model.NormalizeWarnPercent(overlay.CriticalBatteryWarnPercent, 10)
	device.CriticalBatteryWarnText = overlay.CriticalBatteryWarnText
	device.CriticalBatteryWarnPosition = model.NormalizeVerticalBand(overlay.CriticalBatteryWarnPosition)
