...
### Fixed
- **Immich sync works again against Immich v3.** Immich v3 removed the `assets` field from the album-detail endpoint the server used to pull a device's selected album(s), so every album-based sync (the default mode) silently returned zero photos — new photos stopped being pulled in. Album fetching now goes through Immich's own documented replacement (`POST /api/search/metadata` with an `albumIds` filter). While fixing this we found the same search endpoint only returns EXIF (location/description) and face/people data when the request explicitly asks for it — a pre-existing gap, not new to v3 — so those also weren't coming through for the "all photos" / "favorites" sync modes. Both are now requested on every sync, restoring names, location and comments.
- **Frames no longer go permanently silent after a background hiccup.** Several always-on background jobs (MQTT publishing, per-pull device-state updates, Immich/Synology sync, next-image rendering) had no panic recovery — an unhandled panic in *any* of them crashed the entire server process, not just that one task, and with no container restart policy it would then sit dead until someone noticed and restarted it manually. Every background task is now wrapped so a bug in one is logged and contained instead of taking down image serving for every frame.
- **Battery percentage no longer flashes wildly wrong values (2%, 83%, 20%, ...).** Some boards' battery rail briefly sags under WiFi-TX current, landing within the "plausible voltage" range yet implying a wildly different charge level on the steep part of the LiPo curve — and the Devices list / Home Assistant sensor trusted the very latest reading unconditionally (unlike the on-photo badge, which already rode out a single bad reading). A single bad reading could also get misread as a full recharge cycle, silently discarding real drain history. Battery readings are now checked against their neighbors (a small median filter) before being shown or fed into the drain estimate, so an isolated bad wake — or a short run of them — no longer corrupts the displayed percentage, trend, or days-remaining estimate. A device whose readings stay persistently erratic (rather than an occasional blip) is a sign of a real hardware issue (loose battery connector, aging cell) that this can't paper over — it now honestly reports an insufficient trend instead of a fabricated one.

### Added
- **Per-device activity log.** Every image-pull attempt a frame makes — success or failure — is now recorded, unlike the existing "Last check-in" / rotation history which only reflects a successful serve. Open it from the new log icon in the Devices list: timestamp, result, trigger (timer/button/boot), source and battery level, with a configurable retention window (days/months/years, default 6 months — oldest entries are pruned automatically) and a CSV download.
- **See which Immich album(s) a photo — and a frame — actually use.** Photos synced from Immich were previously pooled together with no way to tell which album (and in turn which frame) they came from. The Gallery's Immich tab now has a "Filter by album" chip row (a photo can belong to more than one album, so this is a multi-select, not exclusive tabs) and each thumbnail shows its album membership. The Devices list also now shows which album(s) each frame is scoped to.
